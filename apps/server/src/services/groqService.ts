...
export async function createGroqStream(params: {
  mode: Mode;
  message: string;
}) {
  ...
  if (!process.env.GROQ_API_KEY) {
    throw new Error("缺少 GROQ_API_KEY，请先在 apps/server/.env 中配置");
  }
  ...
  const stream = await client.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: buildSystemInstruction(mode),
      },
      {
        role: "user",
        content: message,
...
export async function testGroqOnce() {
